...
class JobRepository extends BaseRepository {
	...
	async searchJobs(filters = {}, pagination = {}) {
		...
		query += ` ORDER BY j.${sort} ${order.toUpperCase()}`;
		query += ` LIMIT ? OFFSET ?`;
	...
	/**
	 * Get total count of jobs with filters (for pagination)
	 */
	async getJobsCount(filters = {}) {
		const { q, job_name, job_description, company, applied, website, location, ...tagCategoryFilters } = filters;

		let query = `
            SELECT COUNT(DISTINCT j.entity_id) as total
            FROM jobs j
            LEFT JOIN websites w ON j.website_id = w.website_id
            LEFT JOIN companies c ON j.company_id = c.company_id
            LEFT JOIN responses r ON j.response_id = r.response_id
		...
		// Handle tag categories with AND logic - use EXISTS subqueries instead of GROUP BY
		Object.entries(tagCategoryFilters).forEach(([category, tagString]) => {
			if (tagString && typeof tagString === 'string') {
				const tagList = tagString.split(',').map(tag => tag.trim()).filter(tag => tag);
				if (tagList.length > 0) {
					...
					query += ` AND EXISTS (
						...
						JOIN tags t2 ON jt2.tag_id = t2.tag_id 
						WHERE jt2.job_id = j.entity_id 
						AND t2.tag_name IN (${tagPlaceholders})
					)`;
					params.push(...tagList);
				}
			}
