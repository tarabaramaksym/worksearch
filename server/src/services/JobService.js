...
class JobService {
	...
	async searchJobs(filters, pagination = {}) {
		try {
			...
			const jobs = await this.jobRepo.searchJobs(filters, pagination);
			...
			return ResponseFormatter.formatJobsResponse(jobs, filters, paginationMeta);
		} catch (error) {
	...
	/**
	 * Get jobs for filter options calculation (without pagination)
	 */
	async getJobsForFilterOptions(filters = {}) {
		try {
			const allJobs = await this.jobRepo.searchJobs(filters, { page: 1, limit: 10000, sort: 'created_at', order: 'desc' });
			return allJobs;
		} catch (error) {
			throw new Error(`Failed to get jobs for filter options: ${error.message}`);
		}
	}
