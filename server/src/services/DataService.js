 ...
 */
	async createTag(tagData) {
		try {
			...
			const existingTag = await this.tagRepo.findByName(tag_name);
			if (existingTag) {
			...
			const newTag = await this.tagRepo.getOrCreateByName(tag_name, tag_category);
			...
			if (error.message.includes('UNIQUE constraint failed')) {
				const conflictError = new Error('Tag already exists');
